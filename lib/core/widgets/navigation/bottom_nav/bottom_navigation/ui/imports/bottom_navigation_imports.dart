import 'package:flutter/material.dart';
import 'package:mysam_app/app/app_launch/auth/data/models/ui/profile_info.dart';
import 'package:mysam_app/app/app_launch/auth/data/models/ui/user_info.dart';
import 'package:mysam_app/app/app_launch/auth/data/models/ui/user_role_type.dart';
import 'package:mysam_app/core/models/icon_info.dart';
import 'package:mysam_app/core/preferences/preference_manger.dart';
import 'package:mysam_app/core/resources/assets/assets.dart';
import 'package:mysam_app/core/resources/colors/app_colors.dart';
import 'package:mysam_app/core/resources/translation/app_translations.dart';
import 'package:mysam_app/core/widgets/navigation/bottom_nav/bottom_navigation/data/models/custom_navigation_destination_item.dart';
import 'package:mysam_app/core/widgets/place_holder_image.dart';
import 'package:playx/playx.dart';

part '../controller/bottom_navigation_controller.dart';
part '../view/custom_navigation_bar.dart';
part '../view/widgets/build_bottom_nav_profile_image_widget.dart';
